    ...
    return (
      <div className="px-6 py-8 space-y-8 page-transition pb-24 relative">
        ...
        <header className="flex justify-between items-end pt-8">
          <div>
            <h2 className="text-3xl font-black text-black uppercase tracking-tighter">Staff</h2>
        ...
        {isAddingProfesor && (
          <div className="glass-card rounded-2xl p-5 border border-primary/30 space-y-4 shadow-ios">
            <h3 className="text-black font-bold text-sm uppercase tracking-wider">Nuevo Profesor</h3>
            <input 
              type="text" 
              placeholder="Nombre completo" 
              className="w-full bg-ios-gray border-none rounded-xl px-4 py-3 text-sm text-black outline-none focus:ring-2 focus:ring-primary/20"
            ...
            <div className="flex gap-2 pt-2">
              <button 
                ...
                className="flex-1 py-3 rounded-xl border border-black/10 text-secondary font-bold text-xs uppercase tracking-wider"
              >
                Cancelar
              </button>
              <button 
                ...
                className="flex-1 py-3 rounded-xl bg-primary text-white font-bold text-xs uppercase tracking-wider disabled:opacity-50"
              >
        ...
        <div className="space-y-4">
          {profesoresList.map((prof, idx) => {
            ...
            return (
              <div 
                key={prof.id || idx} 
                className="glass-card rounded-3xl p-6 border border-black/5 active:scale-[0.98] transition-all flex flex-col gap-4 group cursor-pointer"
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center border border-primary/20 relative">
                      ...
                      <div className={`absolute -top-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${statusColor}`}></div>
                    </div>
                    <div>
                      <h4 className="font-bold text-black text-lg">{prof.nombre}</h4>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-black/[0.02] rounded-xl p-3 flex flex-col items-center justify-center text-center">
                    <span className="text-black font-black text-sm">{profGrupos.length}</span>
                    <span className="text-[8px] text-secondary uppercase font-bold">Grupos</span>
                  </div>
                  <div className="bg-black/[0.02] rounded-xl p-3 flex flex-col items-center justify-center text-center">
                    <span className="text-black font-black text-sm">{clasesEsteMes}</span>
                    <span className="text-[8px] text-secondary uppercase font-bold">Clases Mes</span>
                  </div>
                  <div className="bg-black/[0.02] rounded-xl p-3 flex flex-col items-center justify-center text-center">
                    <span className="text-black font-black text-sm">{promedioAsistencia}%</span>
                    <span className="text-[8px] text-secondary uppercase font-bold">Asistencia</span>
                  </div>
                </div>

                <div className="flex items-center justify-between pt-2 border-t border-black/5">
                  <span className="text-[10px] text-secondary font-medium uppercase tracking-wider">{alumnosAsignados} Alumnos Asignados</span>
          ...
          {profesoresList.length === 0 && (
            <p className="text-center text-secondary py-10 text-sm italic">No hay profesores registrados.</p>
          )}
        </div>
      </div>
    );
    ...
    return (
      <div className="px-6 py-8 space-y-8 page-transition pb-24">
        <header className="flex items-center gap-4 mb-8">
          <button onClick={() => handleNavigation('Profesores')} className="w-10 h-10 rounded-full bg-ios-gray flex items-center justify-center text-primary border border-black/5 active:scale-90 transition-all">
            <span className="material-icons-outlined">arrow_back</span>
          </button>
          <div className="flex-1">
            <h2 className="text-black font-black text-2xl uppercase tracking-tighter">{selectedProfesor}</h2>
        ...
        <section className="space-y-4">
          <h3 className="text-[10px] uppercase font-black text-secondary tracking-[0.2em] px-1">Estadísticas del Mes</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="glass-card rounded-3xl p-5 border border-black/5 space-y-2">
              <span className="text-[10px] text-secondary uppercase font-bold block">Clases Dictadas</span>
              <div className="flex items-end gap-2">
                <span className="text-3xl font-black text-black">{clasesEsteMes}</span>
            ...
            <div className="glass-card rounded-3xl p-5 border border-black/5 space-y-2">
              <span className="text-[10px] text-secondary uppercase font-bold block">Alumnos Activos</span>
        ...
        {/* Grupos a cargo */}
        <section className="space-y-4">
          <h3 className="text-[10px] uppercase font-black text-secondary tracking-[0.2em] px-1">Grupos a cargo</h3>
          <div className="space-y-3">
            {profGrupos.length > 0 ? profGrupos.map(g => {
              const groupStudents = alumnos.filter(a => a.grupo === g.nombre).length;
              return (
                <div key={g.id} className="glass-card rounded-2xl p-4 border border-black/5 flex justify-between items-center">
                  <div>
                    <h4 className="font-bold text-black text-sm">{g.nombre}</h4>
                    <p className="text-[10px] text-secondary mt-0.5">{g.horario}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-black text-primary block">{groupStudents} Alumnos</span>
                    <span className="text-[8px] text-secondary uppercase font-bold tracking-widest">Inscriptos</span>
                  </div>
                </div>
              );
            }) : (
              <p className="text-sm text-secondary px-1 italic">No tiene grupos asignados actualmente.</p>
            )}
          </div>
        </section>
        ...
        <section className="space-y-4">
          <h3 className="text-[10px] uppercase font-black text-secondary tracking-[0.2em] px-1">Habilidades Trabajadas</h3>
          <div className="flex flex-wrap gap-2">
            {habilidadesTrabajadas.length > 0 ? habilidadesTrabajadas.map((hab, idx) => (
              ...
              <p className="text-sm text-secondary px-1 italic">Sin registro de habilidades este mes.</p>
            )}
          </div>
        </section>
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-end px-1">
            <h3 className="text-[10px] uppercase font-black text-secondary tracking-[0.2em]">Historial de clases</h3>
            <button 
          ...
          <div className="space-y-3">
            {profClases.slice(0, 5).map(clase => (
              <div 
                ...
                className="glass-card rounded-2xl p-5 border border-black/5 flex items-center gap-4 cursor-pointer hover:border-primary/30 transition-all"
              >
                <div className="w-10 h-10 bg-ios-gray rounded-xl flex items-center justify-center text-secondary">
                  <span className="material-icons-outlined text-sm">event</span>
                </div>
                <div className="flex-1">
                  <h4 className="font-bold text-black text-sm">{clase.grupo}</h4>
                  <p className="text-[10px] text-secondary">{new Date(clase.fecha).toLocaleDateString()} • {clase.horario}</p>
                </div>
